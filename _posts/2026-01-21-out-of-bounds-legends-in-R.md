...
Quick aside: as I'm using this data to illustrate a visualization method, I'm intentionally not normalizing streamflow by watershed area, which would of course reduce the skewness of the distribution considerably. Moving on...
...
A common goal is to use the color scale efficiently so that the bulk of observations spans more of the available range, which improves visual discrimination among most stations. Now, since our data are never so perfect as to be uniformly distributed across our scale range, we often have to deal with this type of thing. 

I should note here that there are many different ways we could deal with this issue, including transforming the distribution of the data or using a more complex color space, and in some situations these are fine. However, there are many situations where I would deliberately avoid using some of these strategies, as they can lead to less interpretable plots (for example, I often avoid transforms for maps when I want the legend to remain in the original units).

In this case, let's say we're more interested in allowing someone to distinguish among the values for the bulk of our data. Looking at our histogram, that represents values between about 0 and 1000 m^3/s. An obvious solution is to cap the color scale so the bulk of values use more of the available range, improving visual discrimination where most observations live. In `ggplot2`, you can do this by setting an upper `limits` value. The catch is that, by default, values outside the `limits` become `NA` for the scale and are drawn with `na.value` (often grey), which could be mistaken for missing data. Instead of dropping those values from the mapping, you can _squish_ them: use `oob = scales::oob_squish` (or its alias `scales::squish`) so anything above the upper limit is mapped to the maximum color, and anything below the lower limit is mapped to the minimum color.
...
And of course, thank you to the developer of the `legendry` package. You made my day!